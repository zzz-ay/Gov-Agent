...
GOV-RAG 是一个面向政策咨询、法规解释、合规风险和类案参考的多知识源 RAG Agent Demo 项目。项目从原来的“高校毕业生政务办事助手”升级为更通用的：
...
本项目默认在 Windows + `G:\gov-rag` 路径下运行，并使用项目专用 conda 环境。
...
如果你本机环境名不是 `govrag`，请替换为实际的 gov-rag 项目 conda 环境名。
...
原始项目只围绕高校毕业生政策问答设计。当前版本已经升级为多知识源平台：
...
当前 LeCaRDv2 只保留约 20-50 条代表性案例，适合本地 Demo。不要把 55,000 多条候选案例全部导入 Chroma，否则重建向量库容易卡死。
...
本项目在本地 16GB 内存机器上运行，因此对重任务做了运行时保护：
...
```text
Streamlit 文件监听反复扫描 data/vectorstore
BM25 运行时全量重建导致 OOM
```
...
- 从单一毕业生政策助手升级为政策、法规、案例多知识源 RAG Agent 平台。
